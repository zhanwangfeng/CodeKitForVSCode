...
export function activate(context: vscode.ExtensionContext): void {
  ...
  // —— 编辑器右键：HTML 实体 ——
  context.subscriptions.push(
    vscode.commands.registerCommand('codeKit.openHtmlEntity', () =>
      openToolWithSelection(htmlEntityTool, context),
    ),
    vscode.commands.registerCommand('codeKit.htmlEncode', () => {
      const editor = vscode.window.activeTextEditor;
      if (editor) htmlEncode(editor);
    }),
    vscode.commands.registerCommand('codeKit.htmlDecode', () => {
      const editor = vscode.window.activeTextEditor;
      if (editor) htmlDecode(editor);
    }),
  );

  // —— 编辑器右键：文本统计 ——
  context.subscriptions.push(
    vscode.commands.registerCommand('codeKit.openTextCounter', () =>
      openToolWithSelection(textCounterTool, context),
    ),
  );
