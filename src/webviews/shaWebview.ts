/** SHA 哈希专用 WebView：纵向布局——菜单栏（算法下拉）、哈希行（label+复制+哈希值）、输入标题、输入框，实时计算。 */
import { getLocale, t } from '../i18n';

/** SHA WebView 全部文案（语言切换时由扩展侧重新取值并 postMessage 给 WebView） */
export function getShaUI() {
  return {
    title: t('tool.sha.name'),
    algorithm: t('sha.algorithm'),
    input: t('sha.input'),
    output: t('sha.output'),
...
export function getShaWebviewContent(initialText?: string): string {
  ...
  const algoOptions = algorithms
    .map((a) => `        <option value="${a}"${a === 'SHA-256' ? ' selected' : ''}>${a}</option>`)
    .join('\n');
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .toolbar-actions select:focus { border-color: var(--vscode-focusBorder, #007fd4); }
...
<body>
  <div class="toolbar">
    <button class="title-badge" type="button" disabled data-i18n="title">${t('tool.sha.name')}</button>
    <div class="toolbar-actions">
      <label><span data-i18n="algorithm">${ui.algorithm}</span>
        <select id="algorithm">${algoOptions}
        </select>
      </label>
...
<script>
  ...
  function compute() {
    ...
    try {
      ...
      crypto.subtle.digest(algo, buf).then(function (digest) {
        if (algo !== algoSelect.value) return;
  ...
  // 接收扩展发来的文本，填充到输入窗并触发计算（由 Open xxx 命令发送）
  window.addEventListener('message', function(e) {
