/** 字母大小写转换 WebView：输入文本，实时输出全大写/全小写两种结果。 */
import { getLocale, t } from '../i18n';

/** 字母大小写转换 WebView 全部文案（语言切换时由扩展侧重新取值并 postMessage 给 WebView） */
export function getLetterCaseUI() {
  return {
    title: t('tool.letterCase.name'),
    placeholder: t('letterCase.placeholder'),
...
export function getLetterCaseWebviewContent(initialText?: string): string {
  const langAttr = getLocale() === 'zh-cn' ? 'zh-CN' : 'en';

  const formats = [
    { id: 'upper', label: t('letterCase.upper') },
    { id: 'lower', label: t('letterCase.lower') },
  ];

  const ui = getLetterCaseUI();
...
<title>${t('tool.letterCase.name')}</title>
<style>
...
<body>
  <div class="toolbar">
    <button class="title-badge" type="button" disabled data-i18n="title">${t('tool.letterCase.name')}</button>
...
<script>
  ...
  function update() {
    ...
    ['upper', 'lower'].forEach(function (op) {
  ...
  // 接收扩展发来的文本，填充到输入窗并触发转换（由 Open Letter Case 命令发送）
  window.addEventListener('message', function(e) {
  ...
  // 初始文本（由 Open Letter Case 命令通过 run(context, initialText) 传入）
